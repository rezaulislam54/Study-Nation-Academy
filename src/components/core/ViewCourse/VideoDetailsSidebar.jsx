    ...
    <>
      ...
      >
        <FaAngleDoubleRight
          ...
          className={` md:hidden z-10 cursor-pointer text-2xl text-[#000814] m-2 bg-richblack-100 rounded-full p-1 top-3 absolute -left-1 `}
        />
      </div>
      <div
        className={`${
      ...
      >
        <div
          className={`${
            showSidebar ? "hidden" : ""
          } transition-all origin-right duration-500 flex h-[calc(100vh-3.5rem)] w-[320px] max-w-[350px] flex-col border-r-[1px] border-r-richblack-700 bg-[#161d29] offSidebar2`}
        >
          <div
            className={`${
              showSidebar ? "hidden" : ""
            } mx-5   flex flex-col items-start justify-between gap-2 gap-y-4 border-b border-[#424854] py-5 text-lg font-bold text-richblack-25 offSidebar2`}
          >
          ...
          <div className="h-[calc(100vh - 5rem)] overflow-y-auto px-2">
            {courseSectionData?.map((section, index) => (
              <details
                key={index}
                className=" appearance-none text-[#f1f2ff] detailanimatation"
              >
                <summary className="mt-2 cursor-pointer text-sm text-[#f1f2ff] appearance-none">
                    ...
                    <div
                      ...
                      className={`${
                        subSection?._id === videoActive
                          ? "bg-yellow-200"
                          : "bg-[#f1f2ff]0"
                      } cursor-pointer items-baseline  flex gap-3  px-5 py-2 font-semibold text-[#161d29] relative border-b-[1px] border-[#424854] `}
                    >
      ...
      <div
        ...
        className={`${
          showSidebar ? "hidden" : ""
        } fixed top-0 left-0 w-full h-full bg-[#000814] bg-opacity-50 z-10 offSidebar3`}
      ></div>
    </>
  );
}
